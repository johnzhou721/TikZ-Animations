...
local function is_point_in_triangle(point,triangle)
    ...
    local sign_1 = sign(cross_PQ[1][1])
    local sign_2 = sign(cross_QR[1][1])
    local sign_3 = sign(cross_RP[1][1])
    local sign_4 = sign(cross_PQ[1][2])
    local sign_5 = sign(cross_QR[1][2])
    local sign_6 = sign(cross_RP[1][2])
    local sign_7 = sign(cross_PQ[1][3])
    local sign_8 = sign(cross_QR[1][3])
    local sign_9 = sign(cross_RP[1][3])
    if (
...
function ss.compare_triangles(triangle_1,triangle_2)
    ...
    if #triangle_1.segment == 2 or #triangle_2.segment == 2 then
        local function depth_mid(seg)
            if #seg == 2 then
                ...
                return dot_product(mid, observer)
            else
                ...
                return dot_product(cent, observer)
            end
        end
    ...
    if (P_test or Q_test or R_test) then
        ...
        local normal_1 = cross_product(
            matrix_subtract(Q_1,P_1)
            ,matrix_subtract(R_1,P_1)
        )
        if dot_product(normal_1,observer) < 1 then
            normal_1 = matrix_scale(-1,normal_1)
        end
        ...
        if test == "P" then
            signed_distance_to_plane = norm(
                matrix_add(
                    P_2
                    ,matrix_scale(-1,P_2_projection)
                )
            )
            if (
                dot_product(
                    matrix_subtract(P_2,P_2_projection)
                    ,normal_1
                ) < 1
            ) then 
        ...
        if test == "Q" then
            signed_distance_to_plane = norm(
                matrix_subtract(Q_2,Q_2_projection)
            )
            if (
                dot_product(
                    matrix_subtract(Q_2,Q_2_projection)
        ...
        if test == "R" then
            signed_distance_to_plane = norm(
                matrix_subtract(R_2,R_2_projection)
            )
            if (
                dot_product(
                    matrix_subtract(R_2,R_2_projection)
        ...
        local midpoint_1 = midpoint({P_1[1],Q_1[1],R_1[1]})
        local midpoint_2 = midpoint({P_2[1],Q_2[1],R_2[1]})
