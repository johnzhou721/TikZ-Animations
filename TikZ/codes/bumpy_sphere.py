import numpy as np
import Animation_Modules.animatetex as animatetex

numiter = 24

preamble= r'''
\documentclass{beamer}
\beamertemplatenavigationsymbolsempty

\usepackage{
    tikz
    ,tikz-3dplot
    ,TikZ/packages/tikz_sorting
}

% https://tex.stackexchange.com/a/15449/319072
\pgfmathdeclarefunction{sphereX}{2}{%
    \pgfmathparse{2*cos(#2)*cos(#1)}%
}
\pgfmathdeclarefunction{sphereY}{2}{%
    \pgfmathparse{2*cos(#2)*sin(#1)}%
}
\pgfmathdeclarefunction{sphereZ}{2}{%
    \pgfmathparse{2*sin(#2)}%
}
\pgfmathdeclarefunction{perturbation}{2}{%
    \pgfmathparse{(1/5)*sin(6*#1)*sin(5*#2)}%
}
\pgfmathdeclarefunction{BsphereX}{2}{%
    \pgfmathparse{perturbation(#1,#2)*sphereX(#1,#2)+sphereX(#1,#2)}%
}
\pgfmathdeclarefunction{BsphereY}{2}{%
    \pgfmathparse{perturbation(#1,#2)*sphereY(#1,#2)+sphereY(#1,#2)}%
}
\pgfmathdeclarefunction{BsphereZ}{2}{%
    \pgfmathparse{perturbation(#1,#2)*sphereZ(#1,#2)+sphereZ(#1,#2)}%
}


'''

postscript = r'''
\begin{document}
    \begin{frame}
        \NewSegmentList{bumpysphere}
        \pgfmathsetmacro{\azimuth}{100+\azimuthchange}
        \pgfmathsetmacro{\elevation}{30}
        % https://tex.stackexchange.com/a/729510/319072
        \pgfmathsetmacro{\CameraX}{sin(\azimuth)*cos(\elevation)}
        \pgfmathsetmacro{\CameraY}{-cos(\azimuth)*cos(\elevation)}
        \pgfmathsetmacro{\CameraZ}{sin(\elevation)}
        \tdplotsetmaincoords{90-\elevation}{\azimuth}
        \begin{tikzpicture}[tdplot_main_coords]
            \path[tdplot_screen_coords] (-\textwidth/2,-\textheight/2) rectangle (\textwidth/2,\textheight/2);
            \clip[tdplot_screen_coords] (-\textwidth/2,-\textheight/2) rectangle (\textwidth/2,\textheight/2);

            \pgfmathsetmacro\usamples{20}
            \pgfmathsetmacro\vsamples{20}
            \pgfmathsetmacro\ustart{120}
            \pgfmathsetmacro\uend{330}
            \pgfmathsetmacro\usamplediff{(\uend-\ustart)/\usamples}
            \pgfmathsetmacro\vstart{-60}
            \pgfmathsetmacro\vend{60}
            \pgfmathsetmacro\vsamplediff{(\vend-\vstart)/\vsamples}
            \foreach[parse=true, evaluate=\u] \u in {\ustart,\ustart+\usamplediff,...,\uend-\usamplediff} {
                \foreach[parse=true, evaluate=\v] \v in {\vstart,\vstart+\vsamplediff,...,\vend-\vsamplediff} {
                    % first point
                    \pgfmathsetmacro\ax{BsphereX(\u,\v)}
                    \pgfmathsetmacro\ay{BsphereY(\u,\v)}
                    \pgfmathsetmacro\az{BsphereZ(\u,\v)}

                    % second point
                    \pgfmathsetmacro\bx{BsphereX(\u+\usamplediff,\v)}
                    \pgfmathsetmacro\by{BsphereY(\u+\usamplediff,\v)}
                    \pgfmathsetmacro\bz{BsphereZ(\u+\usamplediff,\v)}

                    % third point
                    \pgfmathsetmacro\cx{BsphereX(\u,\v+\vsamplediff)}
                    \pgfmathsetmacro\cy{BsphereY(\u,\v+\vsamplediff)}
                    \pgfmathsetmacro\cz{BsphereZ(\u,\v+\vsamplediff)}

                    % fourth point
                    \pgfmathsetmacro\dx{BsphereX(\u+\usamplediff,\v+\vsamplediff)}
                    \pgfmathsetmacro\dy{BsphereY(\u+\usamplediff,\v+\vsamplediff)}
                    \pgfmathsetmacro\dz{BsphereZ(\u+\usamplediff,\v+\vsamplediff)}

                    % average 1
                    \pgfmathsetmacro\avgonex{(\ax+\cx+\dx)/3}
                    \pgfmathsetmacro\avgoney{(\ay+\cy+\dy)/3}
                    \pgfmathsetmacro\avgonez{(\az+\cz+\dz)/3}

                    % average 2
                    \pgfmathsetmacro\avgtwox{(\ax+\bx+\dx)/3}
                    \pgfmathsetmacro\avgtwoy{(\ay+\by+\dy)/3}
                    \pgfmathsetmacro\avgtwoz{(\az+\bz+\dz)/3}

                    % depth dot products w.r.t camera vector
                    \pgfmathsetmacro\depthDPone{
                        \CameraX*\avgonex + 
                        \CameraY*\avgoney + 
                        \CameraZ*\avgonez
                    }

                    \pgfmathsetmacro\depthDPtwo{
                        \CameraX*\avgtwox + 
                        \CameraY*\avgtwoy + 
                        \CameraZ*\avgtwoz
                    }

                    % spanning vectors
                    \pgfmathsetmacro\cax{\cx-\ax}
                    \pgfmathsetmacro\cay{\cy-\ay}
                    \pgfmathsetmacro\caz{\cz-\az}

                    \pgfmathsetmacro\dax{\dx-\ax}
                    \pgfmathsetmacro\day{\dy-\ay}
                    \pgfmathsetmacro\daz{\dz-\az}

                    \pgfmathsetmacro\bax{\bx-\ax}
                    \pgfmathsetmacro\bay{\by-\ay}
                    \pgfmathsetmacro\baz{\bz-\az}

                    % normal vectors
                    \tdplotcrossprod(\cax,\cay,\caz)(\dax,\day,\daz)
                    \pgfmathsetmacro\aviewingDP{
                        \tdplotresx*\avgonex + 
                        \tdplotresy*\avgoney + 
                        \tdplotresz*\avgonez
                    }
                    \pgfmathparse{\aviewingDP<0}
                    \ifnum\pgfmathresult=1
                        \tdplotcrossprod(\dax,\day,\daz)(\cax,\cay,\caz)
                    \fi
                    \pgfmathsetmacro\afinalDP{
                        \CameraX*\tdplotresx +
                        \CameraY*\tdplotresy +
                        \CameraZ*\tdplotresz
                    }

                    \tdplotcrossprod(\bax,\bay,\baz)(\dax,\day,\daz)
                    \pgfmathsetmacro\bviewingDP{
                        \tdplotresx*\avgtwox + 
                        \tdplotresy*\avgtwoy + 
                        \tdplotresz*\avgtwoz
                    }
                    \pgfmathparse{\bviewingDP<0}
                    \ifnum\pgfmathresult=1
                        \tdplotcrossprod(\dax,\day,\daz)(\bax,\bay,\baz)
                    \fi
                    \pgfmathsetmacro\bfinalDP{
                        \CameraX*\tdplotresx +
                        \CameraY*\tdplotresy +
                        \CameraZ*\tdplotresz
                    }

                     % add segments to list
                    \ExpandArgs{nne}\AddSegment{bumpysphere}
                    {\depthDPone}
                    {%
                        \def\noexpand\dotproduct{\afinalDP}%
                        \def\noexpand\aax{\ax}%
                        \def\noexpand\aay{\ay}%
                        \def\noexpand\aaz{\az}%
                        \def\noexpand\bbx{\cx}%
                        \def\noexpand\bby{\cy}%
                        \def\noexpand\bbz{\cz}%
                        \def\noexpand\ccx{\dx}%
                        \def\noexpand\ccy{\dy}%
                        \def\noexpand\ccz{\dz}%
                    }%
                    
                    \ExpandArgs{nne}\AddSegment{bumpysphere}
                    {\depthDPtwo}
                    {%
                        \def\noexpand\dotproduct{\bfinalDP}%
                        \def\noexpand\aax{\ax}%
                        \def\noexpand\aay{\ay}%
                        \def\noexpand\aaz{\az}%
                        \def\noexpand\bbx{\bx}%
                        \def\noexpand\bby{\by}%
                        \def\noexpand\bbz{\bz}%
                        \def\noexpand\ccx{\dx}%
                        \def\noexpand\ccy{\dy}%
                        \def\noexpand\ccz{\dz}%
                    }%
                }
            }

            \SortSegmentList{bumpysphere}
            \LoopOverSegmentList{bumpysphere}
                {   
                \pgfmathparse{\dotproduct>=0}
                    \ifnum\pgfmathresult=1
                        \tikzset{myfill/.style =  yellow}
                    \else
                        \tikzset{myfill/.style =  purple!80}
                    \fi
                  \fill[myfill,postaction={draw,black,ultra thin}] 
                  (\aax,\aay,\aaz) -- (\bbx,\bby,\bbz) -- (\ccx,\ccy,\ccz) --cycle;
                  \pgfmathparse{\dotproduct>=0}
                  \ifnum\pgfmathresult=1
                  %\draw[->](\centroidx,\centroidy,\centroidz) -- ++($0.5*(\normalx cm,\normaly cm,\normalz cm)$);
                  \fi
                }
        \end{tikzpicture}
    \end{frame}
\end{document}
'''

def main():
    """
    Purpose:
        This function animates a spinning bumpy sphere.
    Parameters:
        No parameters.
    Return:
        Void.
    """
    animatetex.before_loop()
    for theta in np.linspace(-20,20,numiter):
        with open(animatetex.TeX_file, 'w') as TeX:
            TeX.write(preamble)
            TeX.write(r'\pgfmathsetmacro{\azimuthchange}{' + f'{theta}' + '}')
            TeX.write(postscript)
        animatetex.during_loop()
    animatetex.after_loop()

if __name__ == "__main__":
    main()