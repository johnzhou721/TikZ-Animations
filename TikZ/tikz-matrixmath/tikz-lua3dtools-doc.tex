...
\DeclareCaptionFormat{tikz-lua3dtools}{#1#2\par\noindent#3}
\captionsetup{
    justification = raggedright
    ,singlelinecheck = false
    ,format = tikz-lua3dtools
}
\captionsetup[subfigure]{format=hang}
\setcounter{tocdepth}{3}
\setmainfont{Carlito}
\title{
    The tikz-lua3dtools Package,\\
    Version 1.0.0\\
    {\small https://github.com/Pseudonym321/TikZ-Animations/tree/master1/TikZ/tikz-lua3dtools}
}
\author{Jasper}
\date{\today}
\begin{document}
    ...
    \chapter{Introduction}
        tikz-lua3dtools is a Lua-based 3D graphics package for 
        Ti\textit{k}Z.
        \section{What are the current capabilities of tikz-lua3dtools?}
            tikz-lua3dtools can render multiple non-intersecting 
        ...
        \section{
            How does tikz-lua3dtools improve on existing packages?
        }
            ...
            matrix transformations. tikz-lua3dtools is built to render 
        ...
        \section{Where is the package at, and where it is heading?}
            The vision of the package tikz-lua3dtools is to one 
            ...
            tikz-lua3dtools is to also implement an ODE 
    ...
    \chapter{Geometric Vectors}
        \section{How are geometric vectors defined in tikz-lua3dtools?}
...
\end{minted}
    ...
    \chapter{The Math Module}
        ...
        \mintinline{lua}{math.cos()}. tikz-lua3dtools defines a growing
        ...
        \textbf{tikz-lua3dtools uses row-vector convention!} This means that 
        you put the transformation on the right of the matrix 
        multiplication unlike with column vectors. Additionally,
        \textbf{tikz-lua3dtools uses matrix-vector convention!} This means 
...
\end{minted}
            \begin{figure}
                ...
                \label{fig:3-1-1}
            \end{figure}
            tikz-lua3dtools is also capable of handling division by zero
...
\end{minted}
            \begin{figure}
                ...
                \label{fig:3-1-2}
            \end{figure}
            tikz-lua3dtools is meant to be compatible with Ti\textit{k}Z, and
            they are meant to be used together---at least for now.
            Maybe one day tikz-lua3dtools will have enough capabilities to 
